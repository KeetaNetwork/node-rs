...
fn main() {
	...
	// Fix the broken import paths generated by rasn-compiler
	fix_import_paths();
...
/// Fix incorrect import paths generated by rasn-compiler
fn fix_import_paths() {
	let subject_file = Path::new("generated/subject_public_key_info_definitions.rs");
	if subject_file.exists() {
		let content = fs::read_to_string(subject_file).expect("Failed to read subject_public_key_info_definitions.rs");
		let fixed_content = content.replace(
			"use super::algorithm_identifier_definitions::",
			"use crate::generated::algorithm_identifier_definitions::",
		);
		fs::write(subject_file, fixed_content).expect("Failed to write fixed subject_public_key_info_definitions.rs");
	}
}
