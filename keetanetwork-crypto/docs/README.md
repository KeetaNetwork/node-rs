...
This crate owns algorithm-agnostic primitives for keys, hashes, signatures, and encryption. Account, block, and vote crates sign through these types. They do not embed a second crypto stack.
...
From `keetanetwork-crypto/src/hash.rs` `hash_default` and `keetanetwork-crypto/src/utils.rs` `test_generate_random_seed`.
